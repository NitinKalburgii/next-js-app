...
export default function clp2({ final }: { final: any }) {
  return (
...
export async function getStaticProps(context: any) {
  const pageUrl = context.params.clp2;
  if (!pageUrl.includes('-c2-')) {
    return {
      notFound: true,
    };
  }
  const searchTerm = pageUrl.split('-c2-')[1];
...
export async function getStaticPaths() {
  return {
    paths: [
      { params: { clp2: 'Agriculture-c2-AG00' } },
      { params: { clp2: 'General+Energy-c2-EG00' } },
      { params: { clp2: 'Humanities-c2-HUM00F' } },
    ],
    fallback: false,
  };
}
